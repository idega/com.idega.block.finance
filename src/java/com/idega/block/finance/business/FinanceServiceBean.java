...
public class FinanceServiceBean extends IBOServiceBean implements FinanceService {
	...
	public Period getPeriodByGroupAndDate(Integer groupId, Timestamp timestamp, Boolean controlsMembership) {
		try {
			return getPeriodHome().findByGroupAndDate(groupId, timestamp, controlsMembership);
		} catch (Exception e) {
	...
	public Collection<Period> getAllByGroupAndDatesAndControlsMembershipAndOlderThanGivenPeriodId(Integer groupId, Timestamp timestampFrom, Timestamp timestampTo, Boolean controlsMembership, Integer periodId) {
		try {
			return getPeriodHome().findAllByGroupAndDatesAndControlsMembershipAndOlderThanGivenPeriodId(groupId, timestampFrom, timestampTo, controlsMembership, periodId);
		} catch (Exception e) {
			getLogger().log(Level.WARNING, "Could not get the periods by groupId: " + groupId + ", timestampFrom: " + timestampFrom + ", timestampTo: " + timestampTo + ", controlsMembership: " + controlsMembership + ", periodId: " + periodId, e);
		}
		return null;
	}

	@Override
	public Collection<Period>  getAllByGroupAndDateAndControlsMembership(Integer groupId, Timestamp timestamp, Boolean controlsMembership) {
		try {
			return getPeriodHome().findAllByGroupAndDateAndControlsMembership(groupId, timestamp, controlsMembership);
		} catch (Exception e) {
			getLogger().log(Level.WARNING, "Could not get the period by groupId: " + groupId + " and timestamp: " + timestamp + " and controlsMembership: " + controlsMembership, e);
		}
		return null;
	}
