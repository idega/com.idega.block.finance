...
public class AssessmentBusinessBean extends IBOServiceBean implements AssessmentBusiness {
	...
	public EntryGroup groupEntriesWithSQL(IWTimestamp from, IWTimestamp to, IWTimestamp paymentDate, IWTimestamp dueDate)
			throws Exception {
		javax.transaction.UserTransaction t = getSessionContext().getUserTransaction();
		try {
			t.begin();
			EntryGroup EG = null;
			int gid = -1;
			try {
				...
				EG.setGroupDate(IWTimestamp.RightNow().getDate());
				EG.setFileInvoiceDate(paymentDate.getTimestamp());
				EG.setFileDueDate(dueDate.getTimestamp());
				EG.store();
			...
			if (EG != null) {
				...
				sql.append(" is null ");
				if (from != null) {
					sql.append(" and ").append(dateColummn).append(" >= '").append(from.getDate());
					sql.append(" 00:00:00' ");
				}
				if (to != null) {
					sql.append(" and ").append(dateColummn).append(" <= ");
					sql.append('\'');
					sql.append(to.getDate());
					sql.append(" 23:59:59'");
				}
				String where = " where " + AccountEntryBMPBean.getEntryGroupIdColumnName() + " = " + gid;
				String sMinSql = "select min(" + AccountEntryBMPBean.getEntityTableName() + "_ID" + ") from "
						+ AccountEntryBMPBean.getEntityTableName() + where;
				String sMaxSql = "select max(" + AccountEntryBMPBean.getEntityTableName() + "_ID" + ") from "
						+ com.idega.block.finance.data.AccountEntryBMPBean.getEntityTableName() + where;
	...
	public void groupEntries(IWTimestamp from, IWTimestamp to) throws Exception {
		Collection entries = ((AccountEntryHome) getIDOHome(AccountEntry.class)).findUnGrouped(from.getDate(),
				to.getDate());
		if (entries != null) {
			int min = 0, max = 0;
			EntryGroup EG = null;
	...
	public void assessTariffsToAccount(float price, String name, String info, Integer accountID, Integer accountKeyID,
		...
		try {
			...
			createAccountEntry(accountID, accountKeyID, new Integer(1), ((Integer) AR.getPrimaryKey()), price, 0,
					price, paydate, name, info, "C", externalID);
			if (save) {
	...
	public void assessTariffsToAccount(Integer[] tariffIds, Double[] multiplyFactors, Integer accountID, Date paydate,
			int discount, Integer tariffGroupID, Integer financeCategoryID, Integer externalID, Integer assessmentRound)
	...
	public void assessTariffsToAccount(List tariffs, List multiplyFactors, Integer accountID, Date paydate,
			int discount, Integer tariffGroupID, Integer financeCategoryID, Integer externalID, Integer assessmentRound) {
		javax.transaction.UserTransaction transaction = this.getSessionContext().getUserTransaction();
	...
	private void createAccountEntries(List tariffs, List factors, Integer accountID, Date paydate, int discount,
			Integer roundID, Integer externalID) throws RemoteException, CreateException {
		Tariff tariff;
		Double factor;
	...
	private AssessmentRound createAssessmentRound(Integer accountID, Integer tariffGroupID, Integer financeCategory,
			java.sql.Date duedate) throws IDOLookupException, CreateException {
		AssessmentRoundHome arh = (AssessmentRoundHome) IDOLookup.getHome(AssessmentRound.class);
