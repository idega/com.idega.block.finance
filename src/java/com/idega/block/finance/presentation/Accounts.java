...
public class Accounts extends Finance {
	...
	private PresentationObject getSearchForm(IWContext iwc, int iCategoryId) {
		...
		/*
		 * if (first != null) firstname.setContent(first);
		 * 
		 * if (middle != null) middlename.setContent(middle);
		 * 
		 * if (last != null) lastname.setContent(last);
		 */
	...
	private void performSearch(IWContext iwc, int iCategoryId) {
		...
		// Else we try to lookup by name
			 ...
			 * if (iwc.isParameterSet("sf_firstname")) { first = iwc.getParameter("sf_firstname"); hasSomething = true; } if
			 * (iwc.isParameterSet("sf_middlename")) { middle = iwc.getParameter("sf_middlename"); hasSomething = true; } if
			 * (iwc.isParameterSet("sf_lastname")) { last = iwc.getParameter("sf_lastname"); hasSomething = true; }
			 */
			if (hasSomething) {
				try {
		...
		if (this.accounts != null) {
			...
			while (I.hasNext()) {
				...
				if (this.viewerPageId > 0) {
					...
					accountLink.addParameter(Finance.prmAccountId, A.getAccountId().toString());
