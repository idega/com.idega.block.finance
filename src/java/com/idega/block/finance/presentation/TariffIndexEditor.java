...
public class TariffIndexEditor extends Finance {
	...
	protected PresentationObject makeLinkTable(int menuNr, int iCategoryId) {
		...
		Link1.addParameter(TariffIndexEditor.strAction, String.valueOf(ACT1));
		...
		Link2.addParameter(TariffIndexEditor.strAction, String.valueOf(ACT2));
	...
	protected PresentationObject getChangeTable(IWContext iwc, int iCategoryId) throws SQLException {
		...
		inputTable.add(new HiddenInput(TariffIndexEditor.strAction, String.valueOf(ACT3)));
		...
		for (int i = 1; i < count + 1; i++) {
			...
			try {
				if (ID != null && ID.intValue() > 0) {
					...
					else if (!sName.equalsIgnoreCase(ti.getName()) || !sInfo.equalsIgnoreCase(ti.getInfo()) || !sType.equalsIgnoreCase(ti.getType()) || !(findex == ti.getIndex())) {
	...
	private Collection getIndices(int iCategoryId) {
		try {
			/*
			 * Vector V = new Vector(); for (int i = 0; i < com.idega.block.finance.data.TariffIndexBMPBean.indexType.length(); i++) { TariffIndex ti=
