...
public class AccountTariffer extends Finance {
	...
	private void parse(IWContext iwc) throws java.rmi.RemoteException, CreateException {
		if (iwc.isParameterSet(PARAM_CONFIRM) && iwc.getParameter(PARAM_CONFIRM).equals("true")) {
			...
			if (qtys != null && qtys.length > 0 && ids != null && qtys.length == ids.length) {
				...
				Integer keyId = iwc.isParameterSet(PARAM_ACCOUNT_KEY) ? Integer.valueOf(iwc.getParameter(PARAM_ACCOUNT_KEY)) : null;

				int price = iwc.isParameterSet(PARAM_AMOUNT) ? Integer.parseInt(iwc.getParameter(PARAM_AMOUNT)) : 0;
				if (keyId.intValue() > 0 && price != 0) {
					Integer TariffGroupId = iwc.isParameterSet(PARAM_TARIFF_GROUP_ID) ? Integer.valueOf(iwc.getParameter(PARAM_TARIFF_GROUP_ID)) : null;
	...
	protected void createTariff(IWContext iwc, IWTimestamp paymentDate, Integer tariffkeyID, int amount, Integer tariffGroupID, String name, String info, boolean saveTariff, Integer assessmentRound) throws RemoteException, CreateException {
	...
	private PresentationObject getTariffTable(TariffGroup group) throws java.rmi.RemoteException {
		...
		if (tariffs != null) {
			...
			while (I.hasNext()) {
				...
				CheckBox chk = getCheckBox(PARAM_TARIFF_CHECK, tariff.getPrimaryKey().toString());
				T.add(chk, col++, row);
				if (hasMap) {
				...
				T.add(new HiddenInput(PARAM_TARIFF_IDS, tariff.getPrimaryKey().toString()));
				row++;
			}
	...
	private PresentationObject getTariffPropertiesTable() throws java.rmi.RemoteException {
		...
		col++;
		row = 2;
		SubmitButton confirm = new SubmitButton(this.iwrb.getLocalizedImageButton("fin_confirm", "Confirm"), PARAM_CONFIRM, "true");
