...
public class TariffAssessments extends Finance {
	...
	private void doRollback(IWContext iwc) {
		...
		if (sRoundId != null) {
			...
			try {
				...
				/*
				 * if (rb) status.setMessage(localize("rollback_success", "Rollback was
				 * successfull")); else status.setMessage(localize("rollback_unsuccess",
				 * "Rollback was unsuccessfull"));
				 */
			}
			catch (Exception ex) {
				ex.printStackTrace();
				// status.setMessage(localize("rollback_illegal", "Rollback was
				// illegal"));
			}
		}
	}
		...
		if (iwc.getParameter("pay_date") != null) {
			...
			if (date != null && date.length() == 10) {
				if (roundName != null && roundName.trim().length() > 1) {
					...
					/*
					 * if (assessed) { status.setMessage(localize("assessment_sucess",
					 * "Assessment succeded")); } else {
					 * status.setMessage(localize("assessment_failure", "Assessment
					 * failed")); }
					 */
	...
	protected PresentationObject getActionButtonsTable(IWContext iwc) {
		...
		if (this.isAdmin) {
			...
			// status.setMessageCaller(Link1, localize("view_assessments", "View
			// assessments"));
			// status.setMessageCaller(Link2, localize("make_new_assessment", "Create
			// new assessment"));
			// status.setMessageCaller(Link3, localize("preview_assessment", "Preview
			// assessment"));
		...
		if (this.assessments != null && !this.assessments.isEmpty()) {
			...
			for (Iterator iter = this.assessments.iterator(); iter.hasNext();) {
				...
				if (AR.getStatus().equals(AssessmentStatus.ASSESSED)) {
					...
					// status.setMessageCaller(R, localize("rollback", "Rolls back this
					// assessment"));
		...
		if (this.roundID != null) {
			...
			if (this.accounts != null) {
				...
				for (Iterator iter = this.accounts.iterator(); iter.hasNext();) {
					...
					// T.add(getText(new
					// IWTimestamp(A.getLastUpdated()).getLocaleDate(iwc)), 4, row);
					try {
	...
	private PresentationObject doMainTable(IWContext iwc) {
		...
		// status.setMessageCaller(sb, localize("assessment_time", "Assessment takes
		// time"));
	...
	public void main(IWContext iwc) {
		/*
		 * if (status == null) status = new StatusBar("ass_status"); status
		 * .setStyle("color: #ff0000; font-style: normal; font-family: verdana;
