...
public class TariffAssessments extends Finance {
		...
		if (iwc.getParameter("pay_date") != null) {
			...
			if (date != null && date.length() == 10) {
				if (roundName != null && roundName.trim().length() > 1) {
					...
					handler.executeAssessment(iwc, getFinanceCategoryId(), groupID, roundName, new Integer(1), accKeyId, paydate, startdate, enddate, excessRoundID);
	...
	private PresentationObject doMainTable(IWContext iwc) {
		...
		if (handler != null) {
			try {
				iAccountCount = getFinanceService().getAccountHome().countByTypeAndCategory(handler.getAccountType(), getFinanceCategoryId());
			}
			catch (RemoteException e) {
				e.printStackTrace();
			}
