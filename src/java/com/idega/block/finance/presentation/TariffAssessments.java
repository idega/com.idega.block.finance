...
public class TariffAssessments extends Finance {
	...
	private void doRollback(IWContext iwc) {
		...
		if (sRoundId != null) {
			...
			try {
				...
				/*
				 * if (rb) status.setMessage(localize("rollback_success", "Rollback was successfull")); else status.setMessage(localize("rollback_unsuccess",
				 * "Rollback was unsuccessfull"));
				 */
			}
			catch (Exception ex) {
				ex.printStackTrace();
		...
		if (iwc.getParameter("pay_date") != null) {
			...
			if (date != null && date.length() == 10) {
				if (roundName != null && roundName.trim().length() > 1) {
					...
					this.handler.executeAssessment(iwc, getFinanceCategoryId(), this.groupID, roundName, new Integer(1), accKeyId, paydate, startdate, enddate, excessRoundID);
					debug("Ending Execution " + IWTimestamp.RightNow().toString());
					/*
					 * if (assessed) { status.setMessage(localize("assessment_sucess", "Assessment succeded")); } else {
	...
	/*
	 * private Map mapOfAccountUsers(Integer assessmentRoundId) { // List L = //
	 * FinanceFinder.getInstance().listOfAccountUsersByRoundId(iAssessmentRoundId); Collection users = null; try { users =
	 * getFinanceService().getAccountUserHome().findByAssessmentRound(assessmentRoundId); } catch (RemoteException e) { e.printStackTrace(); } catch
	 * (FinderException e) { e.printStackTrace(); } if (users != null) { Iterator iter = users.iterator(); Map map = new java.util.HashMap();
	 * AccountUser u; while (iter.hasNext()) { u = (AccountUser) iter.next(); map.put(u.getPrimaryKey(), u); } return map; } return null; }
	 */
	...
	private PresentationObject doMainTable(IWContext iwc) {
		...
		if (this.handler != null) {
			try {
				iAccountCount = getFinanceService().getAccountHome().countByTypeAndCategory(this.handler.getAccountType(), getFinanceCategoryId());
			}
			catch (RemoteException e) {
				e.printStackTrace();
			}
		...
		T.setWidth("100%");
		T.add(new HiddenInput(PRM_ACTION, String.valueOf(ACT3)));
	...
	/*
	 * private String getLocalizedAssessmentStatus(String status) { return localize("assessmentstatus." + status, status); }
	 */

	public void main(IWContext iwc) {
		/*
		 * if (status == null) status = new StatusBar("ass_status"); status .setStyle("color: #ff0000; font-style: normal; font-family: verdana;
